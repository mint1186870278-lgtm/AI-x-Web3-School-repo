...
> ⚠️ **核心规则：任何情况下，Agent 完成改动后必须停下来，先展示改动摘要（新增/修改了哪些文件、内容要点），等用户明确说「可以」或「push 吧」之后，才能执行 git push。不能在用户未确认的情况下主动 push。**
...
- 用户说「你来 push 吧」或「可以」才执行 push；用户只说「行」「好」「继续」不算确认
